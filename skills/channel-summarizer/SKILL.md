...
1. Use `run_command` to execute a pre-installed Python script that fetches the channel history via the Slack API `conversations.history`.
2. Extract the `channel_id` from the current message context and pass it as an argument to the script.
3. Read the messages and synthesize a summary for the user.
...
2. Execute the following command using `run_command`:
   ```bash
   python3 /opt/data/custom-skills/channel-summarizer/fetch_history.py <CHANNEL_ID>
   ```
   *(Be sure to replace `<CHANNEL_ID>` with the actual Slack channel ID).*
3. Read the chronological history output from the script.
...
5. If the script outputs an error (such as `missing_scope` or `channel_not_found`), explain the issue directly to the user.
